...
module.exports = async (client, int) => {
    ...
    if (int.isButton()) {
        ...
        if (!int.member.voice.channel)
            return int.reply({ content: `❌ | Ses Kanalına Bağlan`, ephemeral: true, components: [] });

        if (int.guild.members.me.voice.channel && int.member.voice.channelId !== int.guild.members.me.voice.channelId)
            return int.reply({ content: `❌ | Botla aynı ses kanalını girin`, ephemeral: true, components: [] });
        ...
        switch (int.customId) {
            case 'Save Song': {
                int.member.send({ embeds: [embed.Embed_save(track.title, track.url, track.thumbnail, description)] })
                    .then(() => {
                        return int.reply({ content: `✅ | Müzik bilgilerini mesaj olarak gönderdim.`, ephemeral: true, components: [] });
                    })
                    .catch(error => {
                        console.log('error: ' + error);
                        return int.reply({ content: `❌ | Müzik bilgisi gönderilemedi`, ephemeral: true, components: [] });
                    });
            } break;
            ...
            case 'Playing-Loop': {
                ...
                let msg = await int.reply({ content: `Bir döngü modu seçin`, ephemeral: true, components: [row] });
                ...
                collector.on("collect", async i => {
                    ...
                    await i.deferUpdate();
                    await int.editReply({ content: `✅ | bir döngü \`${methods[mode]}\` ayarlanır`, ephemeral: true, components: [] });
                    ...
                    return await queue.dashboard.edit({ embeds: [embed.Embed_dashboard('Müzik bilgisi', track.title, track.url, track.thumbnail, settings(queue))], components: [row] });
                });

                collector.on("end", (collected, reason) => {
                    if (reason == "time" && collected.size == 0) {
                        if (!queue?.deleted && !queue.isPlaying()) queue?.delete();
                        return int.editReply({ content: "❌ | Zaman doldu", ephemeral: true, components: [] });
                    }
                });
            } break;
            ...
            case 'Playing-Stop': {
                ...
                await int.reply({ content: '✅ | Müzik çalmayı durdurdu ve sesten ayrıldı', ephemeral: true, components: [] });
            } break;

            case 'Playing-Shuffle': {
                queue.tracks.shuffle();
                await int.reply({ content: '✅ | Müzik karışık', ephemeral: true, components: [] });
            } break;
        }
    }
    else {
