...
module.exports = {
    ...
    aliases: ['h'],
    showHelp: false,
    description: 'Komut kullanımı',
    usage: 'help [command]',
    options: [
        {
            name: "command",
            description: "Yardıma ihtiyacınız olan komutu girin",
            type: 3,
            required: false
        }
    ],
    ...
    execute(client, message, args) {
        ...
        if (!args[0]) {
            ...
            if (!Boolean(found)) return message.reply({ content: '❌ | Komut bulunamadı', allowedMentions: { repliedUser: false } });
        }
    },
    ...
    slashExecute(client, interaction) {
        ...
        if (!command) {
            ...
            if (!Boolean(found)) return interaction.reply({ content: '❌ | Komut bulunamadı', allowedMentions: { repliedUser: false } });
        }
    },
};
